...
class PokemonDetail : AppCompatActivity() {
   ...
   // private lateinit var binding : PokemonDetailBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       //features2 = findViewById(R.id.ability)
      ...
      var  bundle:Bundle? = intent.extras
       ...
       Glide.with(this).load("$image_url$id.png")
           ...
           .into(main_poke_image)
               ...
               // Picasso.get().load("$image_url$id.png").into(main_poke_image)



        // call the view model function makeApi call
       Toast.makeText(this, "id value$id $", Toast.LENGTH_LONG).show()

        viewModel.makeApiCall(detail_url!!)

       viewModel.pokemon.observe( this , Observer<DetailPokeData> {
           if (it != null) {
              // features.text = it.abilities[1].isHidden.toString()
             //  features2.text = " fetures : ${it.moves[0]}"
               //ability.text = " Ability : ${it.abilities[].ability}"
